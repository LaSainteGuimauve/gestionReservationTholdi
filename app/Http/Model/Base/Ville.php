...
abstract class Ville implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\Http\\Model\\Map\\VilleTableMap';
     ...
     */
    protected $collReservationsRelatedByCodevillemisedisposition;
    protected $collReservationsRelatedByCodevillemisedispositionPartial;
     ...
     */
    protected $reservationsRelatedByCodevillemisedispositionScheduledForDeletion = null;
     ...
     * @return $this|\App\Http\Model\Ville The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $this->codeville = (null !== $col) ? (int) $col : null;
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->aPays = null;
            $this->collReservationsRelatedByCodevillemisedisposition = null;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion !== null) {
                if (!$this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion->isEmpty()) {
                    \App\Http\Model\ReservationQuery::create()
                        ->filterByPrimaryKeys($this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion = null;
                }
            }

            if ($this->collReservationsRelatedByCodevillemisedisposition !== null) {
                foreach ($this->collReservationsRelatedByCodevillemisedisposition as $referrerFK) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'codeVille':
                        $stmt->bindValue($identifier, $this->codeville, PDO::PARAM_INT);
                        break;
                    case 'nomVille':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collReservationsRelatedByCodevillemisedisposition) {
                ...
                $result[$key] = $this->collReservationsRelatedByCodevillemisedisposition->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\App\Http\Model\Ville
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\Http\Model\Ville The current object, for fluid interface
     */
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getReservationsRelatedByCodevillemisedisposition() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addReservationRelatedByCodevillemisedisposition($relObj->copy($deepCopy));
                }
            }
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setCodeville(NULL); // this is a auto-increment column, so set to default value
        }
    }
    ...
    {
        if ('ReservationRelatedByCodevillemisedisposition' === $relationName) {
            $this->initReservationsRelatedByCodevillemisedisposition();
            return;
        }
    ...
    /**
     * Clears out the collReservationsRelatedByCodevillemisedisposition collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return void
     * @see        addReservationsRelatedByCodevillemisedisposition()
     */
    public function clearReservationsRelatedByCodevillemisedisposition()
    {
        $this->collReservationsRelatedByCodevillemisedisposition = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collReservationsRelatedByCodevillemisedisposition collection loaded partially.
     */
    public function resetPartialReservationsRelatedByCodevillemisedisposition($v = true)
    {
        $this->collReservationsRelatedByCodevillemisedispositionPartial = $v;
    }

    /**
     * Initializes the collReservationsRelatedByCodevillemisedisposition collection.
     *
     * By default this just sets the collReservationsRelatedByCodevillemisedisposition collection to an empty array (like clearcollReservationsRelatedByCodevillemisedisposition());
     ...
     * @return void
     */
    public function initReservationsRelatedByCodevillemisedisposition($overrideExisting = true)
    {
        if (null !== $this->collReservationsRelatedByCodevillemisedisposition && !$overrideExisting) {
            return;
        }

        $collectionClassName = ReservationTableMap::getTableMap()->getCollectionClassName();

        $this->collReservationsRelatedByCodevillemisedisposition = new $collectionClassName;
        $this->collReservationsRelatedByCodevillemisedisposition->setModel('\App\Http\Model\Reservation');
    }
     ...
     * @throws PropelException
     */
    public function getReservationsRelatedByCodevillemisedisposition(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collReservationsRelatedByCodevillemisedispositionPartial && !$this->isNew();
        if (null === $this->collReservationsRelatedByCodevillemisedisposition || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collReservationsRelatedByCodevillemisedisposition) {
                    ...
                    $collReservationsRelatedByCodevillemisedisposition = new $collectionClassName;
                    $collReservationsRelatedByCodevillemisedisposition->setModel('\App\Http\Model\Reservation');

                    return $collReservationsRelatedByCodevillemisedisposition;
                }
            } else {
                $collReservationsRelatedByCodevillemisedisposition = ChildReservationQuery::create(null, $criteria)
                    ->filterByVilleRelatedByCodevillemisedisposition($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collReservationsRelatedByCodevillemisedispositionPartial && count($collReservationsRelatedByCodevillemisedisposition)) {
                        $this->initReservationsRelatedByCodevillemisedisposition(false);

                        foreach ($collReservationsRelatedByCodevillemisedisposition as $obj) {
                            if (false == $this->collReservationsRelatedByCodevillemisedisposition->contains($obj)) {
                                $this->collReservationsRelatedByCodevillemisedisposition->append($obj);
                            }
                        }

                        $this->collReservationsRelatedByCodevillemisedispositionPartial = true;
                    }

                    return $collReservationsRelatedByCodevillemisedisposition;
                }

                if ($partial && $this->collReservationsRelatedByCodevillemisedisposition) {
                    foreach ($this->collReservationsRelatedByCodevillemisedisposition as $obj) {
                        if ($obj->isNew()) {
                            $collReservationsRelatedByCodevillemisedisposition[] = $obj;
                        }
                    }
                }

                $this->collReservationsRelatedByCodevillemisedisposition = $collReservationsRelatedByCodevillemisedisposition;
                $this->collReservationsRelatedByCodevillemisedispositionPartial = false;
            }
        }

        return $this->collReservationsRelatedByCodevillemisedisposition;
    }
     ...
     *
     * @param      Collection $reservationsRelatedByCodevillemisedisposition A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildVille The current object (for fluent API support)
     */
    public function setReservationsRelatedByCodevillemisedisposition(Collection $reservationsRelatedByCodevillemisedisposition, ConnectionInterface $con = null)
    {
        /** @var ChildReservation[] $reservationsRelatedByCodevillemisedispositionToDelete */
        $reservationsRelatedByCodevillemisedispositionToDelete = $this->getReservationsRelatedByCodevillemisedisposition(new Criteria(), $con)->diff($reservationsRelatedByCodevillemisedisposition);


        $this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion = $reservationsRelatedByCodevillemisedispositionToDelete;

        foreach ($reservationsRelatedByCodevillemisedispositionToDelete as $reservationRelatedByCodevillemisedispositionRemoved) {
            $reservationRelatedByCodevillemisedispositionRemoved->setVilleRelatedByCodevillemisedisposition(null);
        }

        $this->collReservationsRelatedByCodevillemisedisposition = null;
        foreach ($reservationsRelatedByCodevillemisedisposition as $reservationRelatedByCodevillemisedisposition) {
            $this->addReservationRelatedByCodevillemisedisposition($reservationRelatedByCodevillemisedisposition);
        }

        $this->collReservationsRelatedByCodevillemisedisposition = $reservationsRelatedByCodevillemisedisposition;
        $this->collReservationsRelatedByCodevillemisedispositionPartial = false;
     ...
     * @throws PropelException
     */
    public function countReservationsRelatedByCodevillemisedisposition(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collReservationsRelatedByCodevillemisedispositionPartial && !$this->isNew();
        if (null === $this->collReservationsRelatedByCodevillemisedisposition || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collReservationsRelatedByCodevillemisedisposition) {
                return 0;
            }

            if ($partial && !$criteria) {
                return count($this->getReservationsRelatedByCodevillemisedisposition());
            }
        ...
        return count($this->collReservationsRelatedByCodevillemisedisposition);
    }
     ...
     * @return $this|\App\Http\Model\Ville The current object (for fluent API support)
     */
    public function addReservationRelatedByCodevillemisedisposition(ChildReservation $l)
    {
        if ($this->collReservationsRelatedByCodevillemisedisposition === null) {
            $this->initReservationsRelatedByCodevillemisedisposition();
            $this->collReservationsRelatedByCodevillemisedispositionPartial = true;
        ...
        if (!$this->collReservationsRelatedByCodevillemisedisposition->contains($l)) {
            $this->doAddReservationRelatedByCodevillemisedisposition($l);

            if ($this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion and $this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion->contains($l)) {
                $this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion->remove($this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildReservation $reservationRelatedByCodevillemisedisposition The ChildReservation object to add.
     */
    protected function doAddReservationRelatedByCodevillemisedisposition(ChildReservation $reservationRelatedByCodevillemisedisposition)
    {
        $this->collReservationsRelatedByCodevillemisedisposition[]= $reservationRelatedByCodevillemisedisposition;
        $reservationRelatedByCodevillemisedisposition->setVilleRelatedByCodevillemisedisposition($this);
    }

    /**
     * @param  ChildReservation $reservationRelatedByCodevillemisedisposition The ChildReservation object to remove.
     * @return $this|ChildVille The current object (for fluent API support)
     */
    public function removeReservationRelatedByCodevillemisedisposition(ChildReservation $reservationRelatedByCodevillemisedisposition)
    {
        if ($this->getReservationsRelatedByCodevillemisedisposition()->contains($reservationRelatedByCodevillemisedisposition)) {
            $pos = $this->collReservationsRelatedByCodevillemisedisposition->search($reservationRelatedByCodevillemisedisposition);
            $this->collReservationsRelatedByCodevillemisedisposition->remove($pos);
            if (null === $this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion) {
                $this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion = clone $this->collReservationsRelatedByCodevillemisedisposition;
                $this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion->clear();
            }
            $this->reservationsRelatedByCodevillemisedispositionScheduledForDeletion[]= clone $reservationRelatedByCodevillemisedisposition;
            $reservationRelatedByCodevillemisedisposition->setVilleRelatedByCodevillemisedisposition(null);
        }
     ...
     * been saved, it will retrieve related ReservationsRelatedByCodevillemisedisposition from storage.
     *
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildReservation}> List of ChildReservation objects
     */
    public function getReservationsRelatedByCodevillemisedispositionJoinUtilisateur(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildReservationQuery::create(null, $criteria);
        $query->joinWith('Utilisateur', $joinBehavior);

        return $this->getReservationsRelatedByCodevillemisedisposition($query, $con);
    }
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildReservation}> List of ChildReservation objects
     */
    public function getReservationsRelatedByCodevillemisedispositionJoinDevis(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildReservationQuery::create(null, $criteria);
     ...
     */
    public function resetPartialReservationsRelatedByCodevillerendre($v = true)
    {
     ...
     * @return void
     */
    public function initReservationsRelatedByCodevillerendre($overrideExisting = true)
    {
     ...
     * @throws PropelException
     */
    public function countReservationsRelatedByCodevillerendre(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collReservationsRelatedByCodevillerendrePartial && !$this->isNew();
        if (null === $this->collReservationsRelatedByCodevillerendre || null !== $criteria || $partial) {
     ...
     */
    protected function doAddReservationRelatedByCodevillerendre(ChildReservation $reservationRelatedByCodevillerendre)
    {
    ...
    {
        if ($deep) {
            if ($this->collReservationsRelatedByCodevillemisedisposition) {
                foreach ($this->collReservationsRelatedByCodevillemisedisposition as $o) {
